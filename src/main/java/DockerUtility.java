...
public class DockerUtility {
    ...
    public boolean doesImageExist(DockerImage dockerImage) {
        ...
        if(dockerImage != null) {
            result = dockerClient.listImagesCmd()
                    ...
                    .map(Arrays::asList)
                    .anyMatch(repoTags -> repoTags.contains(dockerImage.getRepository() + ":" + dockerImage.getImageTag()));
        }
        return result;
    }
     ...
     * @param checkAllContainers If true, checks are container names irrespective of whether it is running or not
     ...
     * @param removeContainerIfExists If true, it removes any container with the same name before creating a new one
     */
    public void createContainer(DockerImage dockerImage, String containerName, boolean removeContainerIfExists) {
        // Input validation
        Objects.requireNonNull(dockerImage);
        Objects.requireNonNull(containerName);
        ...
        logger.info("Retrieving port bindings '" + dockerImage.getPortBindings() + "' to add to container '" + containerName + "'");
    ...
    /**
     * Creates and run a docker container from a specified Docker image name with a specified container name.
     * If the image is not present locally, it tries to pull it.
     * @param dockerImage Image to create a container from
     * @param containerName Name of container to create
     * @param removeContainerIfExists If true, it removes any container with the same name before creating a new one
     */
    public void createAndRunContainer(DockerImage dockerImage, String containerName, boolean removeContainerIfExists) throws InterruptedException {
        // Input validation
        Objects.requireNonNull(dockerImage);
        Objects.requireNonNull(containerName);
